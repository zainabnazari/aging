...
# Clustering: Adjusting m to make it less soft (more distinct clusters)
nb_clusters <- 6
m1 <- 1.1  # Lower m value for more distinct (harder) clusters
cl <- mfuzz(exprSet, c=nb_clusters, m=m1)

# Output directory
output <- "/Users/zainabnazari/mfuzz/specific_st/cluster_output_hard"
dir.create(output, showWarnings = TRUE)
membership_cutoff <- 0.7  # Stricter cutoff (for more distinct membership)
...
pdf(file.path(dir, "clusters_cutoff_1_n_6_try_0_h.pdf"), width=10, height=8)
